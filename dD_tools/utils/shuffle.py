...
def get_short():
    ...
    dD_log.debug(f"get_short() for {node.name()} ({node.Class()})")
        ...
        for input_layer, details in layers.items():
            ...
            for output_layer, out_channels in details["out_channels"].items():
                ...
                if short_label:
                    ...
                    label += f"{input_key}➞{input_display}.{in_channels} = {output_display}.{out_channels}\n"

    return label.strip()

#TODO introduce short label
...
def to_label_short(node):
    ...
    node['label'].setValue(label)
    dD_log.debug(f"Label Short: {label}")
